...
> Vendored integration note: this copy is pinned in `VENDORED_FROM.md` and adds
> a CubeCL CUDA backend plus Windows `staticlib`/`cdylib` artifacts. The
> upstream HIP validation below remains useful evidence, but CUDA must pass the
> local `cuda_current_support` and captured real-cloud gates before becoming the
> default ScrollFiesta MLS implementation.
...
Set `MLS_BACKEND=rust-cpu`, `cubecl-cpu`, `cubecl-hip`, or `cubecl-cuda` before calling the
...
For the vendored Windows CUDA path, run the repository-level scripts:

```powershell
scripts\install_rust_local.ps1
scripts\build_mls_cubecl.ps1 -Backend cuda
msbuild cube_mesh.vcxproj /p:Configuration=Release /p:Platform=x64 /p:UseCubeClMls=true
```

The ordinary MSVC build continues to compile `src/common/mls_project.c` unless
`UseCubeClMls=true` is supplied.
