...
/* 1 iff the most recent CubeCL MLS call on this thread succeeded. The legacy
 * MLS_project_verts ABI is void, so C integrations must check this after each
 * projection and fail the enclosing cube when it returns 0. */
int MLS_cubecl_last_call_ok(void);

/* Run a tiny numerical projection through the selected backend. Call once at
 * process startup; this catches asynchronous worker failures such as a missing
 * NVRTC DLL before the legacy void ABI can continue with invalid outputs. */
