...
pub fn project_cubecl_cuda(
    arena: &MlsArena,
    vertices: &[f32],
    radius: f32,
) -> Result<ProjectionResult, String> {
    cube_backend::project_cuda(arena, vertices, radius)
}
...
fn requested_backend(arena_backend: Backend) -> Result<Backend, String> {
    ...
    if let Ok(value) = std::env::var("MLS_BACKEND") {
        return match value.trim().to_ascii_lowercase().as_str() {
            ...
            "cuda" | "cube-cuda" | "cubecl-cuda" => Ok(Backend::CubeCuda),
...
fn auto_backend() -> Backend {
    #[cfg(feature = "cubecl-cuda")]
    {
        return Backend::CubeCuda;
    }
    #[cfg(all(not(feature = "cubecl-cuda"), feature = "cubecl-hip"))]
    {
        return Backend::CubeHip;
    }
    #[cfg(all(
        not(feature = "cubecl-cuda"),
...
fn dispatch_with_normals(
    ...
    match backend {
        ...
        Backend::CubeCuda => {
            #[cfg(feature = "cubecl-cuda")]
            {
                cube_backend::project_cuda_with_normals(arena, vertices, radius, compute_normals)
            }
            #[cfg(not(feature = "cubecl-cuda"))]
            {
                Err(
                    "CubeCL CUDA backend was not compiled; build with --features cubecl-cuda"
                        .into(),
                )
            }
        }
...
/// Returns one when the most recent call on this thread succeeded. This lets
/// legacy C callers fail closed even though `MLS_project_verts` has a void ABI.
#[unsafe(no_mangle)]
pub extern "C" fn MLS_cubecl_last_call_ok() -> i32 {
    LAST_ERROR.with(|slot| i32::from(slot.borrow().as_bytes() == b"no error"))
}

/// Exercise the selected backend on a tiny plane and validate the result.
/// CubeCL executes through worker threads, so a missing NVRTC may be reported
/// there without unwinding through the FFI caller. Numerical validation here
/// turns that asynchronous failure into a reliable process-startup gate.
#[unsafe(no_mangle)]
pub extern "C" fn MLS_cubecl_preflight() -> i32 {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<(), String> {
        let mut points = Vec::with_capacity(7 * 7 * 3);
        for y in -3..=3 {
            for x in -3..=3 {
                points.extend_from_slice(&[0.0_f32, y as f32, x as f32]);
            }
        }
        let projected =
            project_scrollfiesta_one_pass(&points, 4.0, [-4.0, -4.0, -4.0], Backend::Auto, true)?;
        let center = (3 * 7 + 3) * 3;
        let finite = projected
            .vertices
            .iter()
            .chain(&projected.normals)
            .all(|value| value.is_finite());
        if projected.vertices.len() != points.len()
            || projected.normals.len() != points.len()
            || !finite
            || projected.vertices[center].abs() > 1.0e-4
            || projected.normals[center].abs() < 0.9
        {
            return Err("CubeCL preflight produced an invalid plane projection; CUDA/NVRTC may be unavailable".into());
        }
        Ok(())
    }));
    match result {
...
/// Bitmask: bit 0 Rust CPU, bit 1 CubeCL CPU, bit 2 CubeCL HIP, bit 3 CubeCL CUDA.
#[unsafe(no_mangle)]
...
pub extern "C" fn MLS_cubecl_backend_name(backend: u32) -> *const c_char {
    ...
    static CUBE_CUDA: &[u8] = b"cubecl-cuda\0";
