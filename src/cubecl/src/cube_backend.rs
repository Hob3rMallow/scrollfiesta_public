...
pub fn project_cuda(
    ...
    project_with_runtime::<CudaRuntime>(&CudaDevice::default(), arena, vertices, radius, true)
}

/// Default wgpu device, optionally forcing a specific graphics API via the
/// `CUBECL_WGPU_BACKEND` env var (`gl` / `dx12` / `vulkan` / `metal`). cubecl's
/// own `AutoGraphicsApi` override is `#[cfg(test)]`-only; pre-registering the
/// device client with `init_setup::<G>` makes backend selection work in normal
/// builds — needed e.g. to reach a real GPU via the d3d12→GL path under WSL2,
/// where the only Vulkan ICD available is software (lavapipe).
#[cfg(feature = "cubecl-wgpu")]
fn wgpu_device() -> cubecl::wgpu::WgpuDevice {
    use cubecl::wgpu::{Dx12, Metal, OpenGl, Vulkan, WgpuDevice, init_setup};
    use std::sync::Once;
    // init_setup registers the device's client exactly once; guard so repeated
    // projections don't re-register (which panics with "already initialized").
    static INIT: Once = Once::new();
    let device = WgpuDevice::default();
    INIT.call_once(|| {
        if let Ok(backend) = std::env::var("CUBECL_WGPU_BACKEND") {
            match backend.trim().to_ascii_lowercase().as_str() {
                "gl" | "opengl" => {
                    let _ = init_setup::<OpenGl>(&device, Default::default());
                }
                "dx12" => {
                    let _ = init_setup::<Dx12>(&device, Default::default());
                }
                "vulkan" => {
                    let _ = init_setup::<Vulkan>(&device, Default::default());
                }
                "metal" => {
                    let _ = init_setup::<Metal>(&device, Default::default());
                }
                other => eprintln!("ignoring unknown CUBECL_WGPU_BACKEND={other:?}"),
            }
        }
    });
    device
...
pub fn project_wgpu(
    ...
    project_with_runtime::<WgpuRuntime>(&wgpu_device(), arena, vertices, radius, true)
}
...
#[cfg(feature = "cubecl-cuda")]
pub(crate) fn project_cuda_with_normals(
    arena: &MlsArena,
    vertices: &[f32],
    radius: f32,
    compute_normals: bool,
    ...
    use cubecl::cuda::{CudaDevice, CudaRuntime};
    project_with_runtime::<CudaRuntime>(
        &CudaDevice::default(),
        arena,
        vertices,
        radius,
...
#[cfg(feature = "cubecl-wgpu")]
pub(crate) fn project_wgpu_with_normals(
    arena: &MlsArena,
    vertices: &[f32],
    radius: f32,
    compute_normals: bool,
    ...
    use cubecl::wgpu::WgpuRuntime;
    project_with_runtime::<WgpuRuntime>(&wgpu_device(), arena, vertices, radius, compute_normals)
}
