...
meshes â€” one per connected component. This is the foundation of the entire
...
**Input**: Multi-page TIFF (`uint8 [DÃ—HÃ—W]`, values 0 or 1).
**Output**: Array of `(component_id, vertices[NÃ—3], faces[MÃ—3])` tuples, plus
...
1. Load multi-page TIFF via `TiffIO_load()` into a flat `uint8 *vol` of size
   `D * H * W`, arena-allocated. [PIPELINE_REFERENCE Â§4, step 1â€“2]
...
4. BFS flood fill: iterate all voxels in linear order. For each unvisited
   ...
   (face-adjacent only: Â±z, Â±y, Â±x). Count voxels per component.
   [CLRS Ch.22.2; Skiena Ch.7.7; c-style-guide Â§6.2.1]
5. Filter: discard components with fewer than `MIN_CC_SIZE` (500) voxels.
   [PIPELINE_REFERENCE Â§4, step 4]
6. Sort remaining components by voxel count descending. Keep top
   `MAX_COMPONENTS` (20). [PIPELINE_REFERENCE Â§4, step 5]
...
7. **Extract bounding box** of the component from the label volume. Pad by 1
   ...
   `uint8 *padded` subvolume of size `(dz+2) Ã— (dy+2) Ã— (dx+2)` from the
   arena. Copy component voxels into it. The padding ensures marching cubes
   can produce triangles at the boundary.
   [PIPELINE_REFERENCE Â§4, step 6aâ€“6b; c-style-guide Â§6.3.2]
...
8. **Run marching cubes** on the padded subvolume with `level=0.5`. Use the
   ...
   interpolate edge vertices, emit triangles. Output: `float *verts` (NÃ—3)
   and `int32_t *faces` (MÃ—3), 0-indexed.
   [PIPELINE_REFERENCE Â§4, step 6b; c-style-guide Â§6.3.1]
...
9. **Undo padding offset**: subtract 1 from each vertex coordinate, then add
   ...
   [PIPELINE_REFERENCE Â§4, step 6c]

### Phase 4: Backface Cull
10. **Compute PCA normal** of the component's vertex cloud. Build the 3Ã—3
    covariance matrix of vertex positions (subtract mean first). Find the
    eigenvector corresponding to the smallest eigenvalue â€” this is the sheet
    normal direction. Use a direct 3Ã—3 symmetric eigensolver (Jacobi rotation
    ...
    **Sign convention**: flip the eigenvector so its largest-magnitude component
    is positive. [PIPELINE_REFERENCE Â§4, step 6d]

11. **Per-face normal**: for each triangle `(a, b, c)`, compute
    `n = (verts[b] - verts[a]) Ã— (verts[c] - verts[a])`. No normalization
    needed â€” only the sign of `dot(n, pca_normal)` matters.
    [Abrash Ch.51 â€” compute minimum for a binary decision]
...
12. **Keep front faces**: retain face `i` iff `dot(face_normal[i], pca_normal) > 0`.
    ...
    Compact the face array in-place. [PIPELINE_REFERENCE Â§4, step 6d]
...
13. **Mesh connected components via Union-Find**: iterate faces; for each
    ...
    [CLRS Ch.21.3; Skiena Ch.8.1.3; c-style-guide Â§7.3.3]
    ...
    **Why Union-Find instead of BFS**: the face list gives us edges
    ...
    a single pass over the faces â€” no need to build an explicit adjacency graph
    first. This replaces Python's `trimesh.split()` â€” see PIPELINE_REFERENCE
    Â§4 Cleanup for the Python approach.
    [Skiena Â§18.1; CLRS Ch.23.2 (Kruskal pattern)]
...
15. **Micro-hole fill**: find boundary edges (edges appearing in exactly 1
    triangle). Chain into closed loops. For loops with â‰¤ `CLEANUP_MICRO_HOLE_MAX`
    (6) vertices:
    - Loops with â‰¤ 4 verts: fan-triangulate from the first vertex.
    - Loops with 5â€“6 verts: insert centroid, connect each boundary edge to centroid.
    - Match orientation to the adjacent face's half-edge direction.
    [PIPELINE_REFERENCE Â§4, Micro-hole filling algorithm]
    ...
    **Boundary edge detection**: build a hash table or sorted edge list. For
    ...
    Count > 2 means non-manifold â€” log a warning but continue.
    [PIPELINE_REFERENCE Â§14 â€” boundary detection pattern]
...
- **Skiena Ch.7.6.1**: customize traversal via callbacks â€” the same BFS core
...
- **CLRS Ch.21.3**: Union-Find with union-by-rank and path compression,
  O(Î±(N)) amortized. Used for mesh-level connected components in Phase 5.
...
- **Bentley Ch.7**: feasibility check arithmetic. 320Â³ voxels Ã— ~50 ns/voxel
...
- **Nocedal & Wright Ch.5.1**: context for iterative methods, though the 3Ã—3
  eigensolver is direct (not iterative). The PCA is a direct eigenvalue
  decomposition of a 3Ã—3 symmetric matrix â€” use the closed-form cubic formula
  or 3â€“5 Jacobi rotations.
- **Skiena Ch.16.1** (numerical stability): test with known covariance matrices.
  Degenerate cases: all vertices coplanar (one eigenvalue â‰ˆ 0), all vertices
  collinear (two eigenvalues â‰ˆ 0).

### Micro-Hole Fill (Phase 5)
- **PIPELINE_REFERENCE Â§4**: boundary edge chaining, fan triangulation for
  small loops, centroid insertion for 5â€“6 vertex loops.
- **PIPELINE_REFERENCE Â§14**: boundary detection via edge occurrence counting.
  The same sparse-matrix pattern used throughout the pipeline.

### Memory Management (all phases)
- **Hanson Ch.6**: arena allocation. One arena per volume. All Phase 2â€“5
...
- **Sanglard Ch.2**: DOOM's zone allocator â€” the historical precedent for
  arena allocation with purge tags. The pipeline's arena marks serve the
  same role as DOOM's `PU_LEVEL` / `PU_CACHE` tags.
- **Bentley Ch.10**: malloc overhead is 36â€“47 bytes per allocation. Arena
  eliminates this. Never use per-vertex or per-face malloc.

### Performance Framework
- **Abrash Ch.1â€“3**: measure first, then optimize. Profile Step 0 in isolation
  before optimizing. The Python bottleneck (trimesh split, 15â€“45s) is Python
  overhead that vanishes in C. The C bottleneck may be marching cubes or I/O.
- **Bentley Ch.6**: Appel's design levels â€” attack in order: problem definition,
...
### Module Design
- **Hanson Ch.4â€“6**: opaque types, arena parameters, `sizeof *p` allocation.
- **Elements of Style Ch.2â€“3**: one module does one thing. Don't combine TIFF
...
- **TPOP Ch.4**: pass everything explicitly. The arena, the volume, the
  dimensions â€” all explicit parameters, no globals.
...
**Step 0 budget**: ~10â€“15s (combining mesh extraction 5â€“8s + cleanup 15â€“45s
...
|---|---|---|
| TIFF load (320Â³ uint8) | 32.8 MB sequential read | ~0.1â€“0.3s |
| Threshold | 32.8M comparisons | ~0.05s (memory-bound) |
| BFS flood fill (6-conn) | 32.8M voxels, each visited once | ~0.5â€“1.0s |
| Component filtering + sort | â‰¤20 components, O(N) scan | negligible |
| Marching cubes (per comp) | ~50 ns/voxel Ã— up to 32.8M total | ~1.6s total |
| Backface cull | M faces Ã— 1 dot product | ~0.01s per component |
| Union-Find mesh CC | 3 unions per face Ã— M faces | ~0.01s per component |
| Vertex reindex | O(N) scan + remap | ~0.01s per component |
| Micro-hole fill | O(boundary edges), typically small | ~0.01s per component |
| OBJ write (optional) | Text I/O, buffered | ~0.1â€“0.5s per component |
| **Total** | | **~3â€“5s** |

**Verdict**: Step 0 fits easily within budget. The Python profile shows 5â€“8s
for extraction + 15â€“45s for cleanup, but the cleanup cost is almost entirely
Python/trimesh overhead. In C with Union-Find instead of trimesh.split(), the
cleanup becomes O(M) â€” negligible. The dominant cost is marching cubes (~1.6s)
and BFS flood fill (~0.5â€“1.0s).

### Scaling: 7950X â†’ Kaggle
- 7950X single-core is ~2.6Ã— faster than Kaggle (5.7/2.2 GHz, plus IPC and
  cache advantages).
...
- With 2 threads on Kaggle processing 2 components simultaneously, the per-
  component work overlaps. But with â‰¤20 components and the largest dominating,
  expect ~1.3â€“1.5Ã— speedup from 2 threads.
- **Kaggle estimate**: 3â€“5s. Well within the 10â€“15s allocation.

### Does it fit in memory?
- Input volume: 32.8 MB (uint8, 320Â³)
- Label volume: 131 MB (int32, 320Â³) â€” **this is the largest single allocation
  in Step 0**. Consider using `int16_t` if component count â‰¤ 32767 (we keep
  at most 20), saving 65 MB. Or use a `uint16_t` label volume (65 MB).
- Padded subvolume per component: variable, but â‰¤ 35 MB for a full-volume
  component
- Vertex array (100K verts Ã— 3 Ã— float32): 1.2 MB
- Face array (200K faces Ã— 3 Ã— int32): 2.4 MB
- BFS queue: up to 32.8M entries Ã— 4 bytes = 131 MB in worst case (all
  voxels foreground). **Optimization**: use a `uint32_t` packed index
  (z*H*W + y*W + x fits in uint32 for volumes up to ~1625Â³).
- Total peak: ~200â€“300 MB. Fits in Kaggle's ~13 GB.
...
**Â§1.1 Arena Architecture**: Allocate the entire Step 0 working set from the
per-volume arena. The BFS queue, label volume, padded subvolumes, vertex arrays,
face arrays â€” all arena-allocated. No `malloc` calls except inside libtiff
...
**Â§1.3 Allocation Macros**: Use `ARENA_CALLOC(arena, D*H*W, sizeof(int32_t))`
...
**Â§1.6 Stack Hazards**: Never put the volume or label array on the stack.
`uint8_t vol[320][320][320]` is 32 MB â€” instant stack overflow.

**Â§2.1 Module Structure**: `mesh_extract.h` exposes one public function and the
...
**Â§4.6.1 Dense Arrays**: Vertices as `float verts[N*3]`, faces as
`int32_t faces[M*3]`. Not arrays of structs with pointers. Not linked lists.
Sequential access for cache-friendly processing.

**Â§5.1 Feasibility Checks**: The back-of-envelope above confirms Step 0 fits.
Always re-verify if volume dimensions change (400Â³ volumes would quadruple BFS
time but still fit).

**Â§5.4 Cache Locality**: The BFS flood fill accesses 6-neighbors per voxel.
In row-major layout, Â±x neighbors are adjacent in memory (stride 1). Â±y
neighbors have stride W (320). Â±z neighbors have stride H*W (102,400). The z-
...
**Â§5.6 Threading**: Parallelize per-component marching cubes with OpenMP:
```c
...
**Â§6.1 Volume Layout**: `vol[z*H*W + y*W + x]`. Coordinate convention `(z,y,x)`.
Document everywhere.

**Â§6.2 Connected Components**: BFS with flat arena-allocated queue. 6-connectivity.
...
**Â§9.3 Defensive Programming**: Every function handles the empty case. Zero
foreground voxels â†’ return empty component list. Zero faces from marching cubes
â†’ skip component. All face indices validated: `assert(faces[i] >= 0 &&
faces[i] < n_verts)`.

**Â§9.4 Float Discipline**: PCA eigenvector computation uses `double` internally
for numerical stability (3Ã—3 covariance accumulation over 100K+ vertices).
Convert to `float` for the final normal vector. Vertex positions are `float`
throughout.

**Â§9.5 UB Avoidance**: Volume index arithmetic uses `size_t`:
`size_t idx = (size_t)z * H * W + (size_t)y * W + (size_t)x`. Never compute
`z*H*W` as `int` â€” 320Ã—320Ã—320 = 32.8M, fits int32 but leaves no headroom.

**Â§11.2 libtiff**: Wrap in `TiffIO_load()`. Serialize behind a mutex (libtiff
...
```c
/* Component descriptor â€” output of Step 0.
...
- `uint8_t *padded_subvol` â€” per-component padded binary subvolume
- `int32_t *labels` â€” voxel-level component labels (or `uint16_t` for memory)
- `uint32_t *bfs_queue` â€” flat queue, max size = DÃ—HÃ—W (packed linear index)
- `int32_t *comp_sizes` â€” voxel count per component (temporary for sorting)

**Vertex storage note**: The Python pipeline uses `float64` for vertices. The
C port uses `float32`. The SurfaceDice tolerance is Ï„=2.0 voxels. Float32 has
~7 decimal digits of precision, giving sub-microvoxel accuracy. Float64 is
wasted precision that halves cache utilization. See c-style-guide Â§5.4.6â€“5.4.7.

**Face index storage note**: Python uses `int64`. The C port uses `int32_t`.
A mesh with 2M vertices fits int32 (max 2.1B). A single 320Â³ component
will never exceed ~2M vertices. If somehow a 512Â³ volume produces a mesh
...
```c
/*
 * MeshExtract_run â€” Step 0 entry point.
 *
...
- **Call pattern**: `TIFFOpen(path, "r")` â†’ loop over pages with
  `TIFFReadScanline()` or `TIFFReadEncodedStrip()` â†’ `TIFFClose()`.
...
- **Performance**: sequential read of 32.8 MB. Not a bottleneck (~0.1â€“0.3s).
...
3. **Per-component face count after backface cull**: should be approximately
   ...
   computation may flip a few faces). Verify that the cull removes ~45â€“55% of
   faces consistently.
...
4. **PCA normal direction**: compare `dot(c_normal, python_normal)`. Should be
   ...
   convention is flipped â€” catastrophic for downstream steps.
...
5. **Mesh validity invariants** (check programmatically after every run):
   ...
   - Boundary edge count after micro-hole fill â‰¤ boundary count before
     (holes filled, not created).

### Boundary-Value Tests
[c-style-guide Â§10.1.4; Elements Ch.4; TPOP Ch.6]

- **Empty volume** (all zeros): should return 0 components, no crash.
- **Single voxel** (1Ã—1Ã—1, value=1): marching cubes on a 3Ã—3Ã—3 padded volume.
  May produce 0 faces (single voxel below MC threshold). Handle gracefully.
- **All-ones volume** (320Â³ all foreground): one giant component. Marching cubes
...
- **Volume dimensions not 320Â³**: test 256Â³, 400Â³, 128Ã—256Ã—320 (non-cubic).
...
### Stress Tests
[TPOP Ch.6 â€” machine-generated input]

- Random binary volumes of sizes 1Â³ through 512Â³.
...
The padded subvolume is `(dz+2) Ã— (dy+2) Ã— (dx+2)`. Vertex positions from
...
all vertices by 1 voxel â€” invisible unless you compare against Python output.
[Elements Ch.4 â€” off-by-one; c-style-guide Â§9.3.3]
...
keeps faces whose normal dots **positively** with the PCA normal â€” a sign flip
...
can verify by hand. [PIPELINE_REFERENCE Â§4, step 6d]
...
the final evaluation. This is by design â€” the pipeline operates on 6-connected
...
[PIPELINE_REFERENCE Â§4]

### Integer Overflow in Index Arithmetic
`z * H * W` for z=319, H=320, W=320: 319 Ã— 320 Ã— 320 = 32,665,600. This fits
`int32_t` but `z * H * W + y * W + x` for the maximum voxel is 32,767,999 â€”
still fits int32 but barely. For a 512Â³ volume: 511 Ã— 512 Ã— 512 = 134,086,656.
...
(cross product magnitude < Îµ) and remove them. This prevents NaN in normal
computation downstream. [Elements Ch.4 â€” validate output]

### BFS Queue Sizing
In the worst case (entire volume is foreground), the BFS queue needs DÃ—HÃ—W
entries. For a 320Â³ volume, that's 32.8M Ã— 4 bytes = 131 MB. This is large
...
accumulator (6 unique elements of the 3Ã—3 symmetric matrix) and convert the
resulting eigenvector to `float`. The mean should also be computed in `double`.
[c-style-guide Â§9.4; Nocedal â€” numerical hygiene]
