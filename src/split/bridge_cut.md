...
severs thin "bridges" â€” spurious connections between adjacent scroll wraps
...
The Oracle (Step 1) identifies components with â‰¥ 2 sheets. Bridge cut
attempts to separate them by finding the minimum vertex cut â€” the thinnest
...
**Input**: a multi-sheet `ComponentMesh` from Step 0 (oracle sheet count â‰¥ 2).
...
1. **PCA normal and height.** Compute the PCA normal (smallest-eigenvalue
   ...
   this normal â†’ scalar value per vertex. `height` = max projection âˆ’ min
   projection. This is the extent of the mesh along the sheet-stacking
   direction.
   [PIPELINE_REFERENCE Â§6; reuse PCA from Step 0 if available]
...
3. **k-ring BFS expansion.** Expand source seed to `SEED_RING=20` geodesic
   ...
   after overlap removal, the mesh is too small or too connected â€” return
   NO BRIDGE.
   [PIPELINE_REFERENCE Â§6, step 5â€“6; CLRS Â§22.2 for BFS]
...
4. **Build vertex-split flow network.** The key transformation: convert the
   ...
   - Internal edge: `(v_in â†’ v_out)` with capacity 1.
     Exception: source/sink seed vertices get capacity INF.
   - Reverse edge: `(v_out â†’ v_in)` with capacity 0.

   For each mesh edge (u, v) (undirected, from face adjacency):
   - Forward: `(u_out â†’ v_in)` with capacity INF. Reverse: `(v_in â†’ u_out)` cap 0.
   - Forward: `(v_out â†’ u_in)` with capacity INF. Reverse: `(u_in â†’ v_out)` cap 0.
   ...
   ~2.6M edges.
   [CLRS Â§26 Problem 26-1; Skiena Â§18.8; c-style-guide Â§7.5.1â€“7.5.2]
...
5. **Edmonds-Karp max-flow.** Standard BFS-based augmenting-path algorithm.
   ...
   while total_flow < MAX_FLOW_LIMIT (100):
       ...
       trace path Sâ†’T via parent_edge[], find bottleneck
   ...
   worst-case Edmonds-Karp is O(VEÂ²) â€” intractable for 400K nodes Ã— 2.6M
   edges. With the cap, we do at most 100 BFS passes through a 2.6M-edge
   graph, giving ~260M operations â€” feasible.
   [CLRS Â§26.2 Theorem 26.8; c-style-guide Â§7.4.1â€“7.4.6]

6. **Min-cut extraction.** After Edmonds-Karp terminates, the last failed
   BFS's visited set IS the S-side of the min-cut. No separate BFS needed â€”
   ...
   [CLRS Â§26.2 Theorem 26.6; c-style-guide Â§7.6.1]
...
8. **Bridge decision (`is_bridge`).**
   ```
   if split_ratio < 0.20:          â†’ NO BRIDGE (too asymmetric â€” one side
                                      is tiny, this is a fragment not a bridge)
   if flow_height_ratio â‰¤ 0.36:    â†’ BRIDGE (thin neck relative to extent)
   if flow_height_ratio â‰¤ 1.3
      AND split_ratio â‰¥ 0.40
      AND height â‰¥ 20:             â†’ BRIDGE (gray zone â€” moderately thin,
                                      but balanced and tall enough)
   else:                           â†’ NO BRIDGE
   ```
   [PIPELINE_REFERENCE Â§6, is_bridge rule]
...
1. **Find boundary vertices.** Iterate mesh edges; a vertex v is a boundary
   ...
   [PIPELINE_REFERENCE Â§6, split_mesh step 1]

2. **Build KD-tree on boundary vertices.** The boundary set is typically
   small (~100â€“2K vertices). A KD-tree on this set enables fast nearest-
   boundary-distance queries. Alternatively, brute-force is acceptable for
   |boundary| < 2000 (2000Â² = 4M comparisons at ~1ns = 4ms).
   [PIPELINE_REFERENCE Â§6, split_mesh step 2; Skiena Â§15.6]
...
3. **Compute exclusion zone.** For every mesh vertex, query its distance to
   ...
   [PIPELINE_REFERENCE Â§6, split_mesh step 3â€“4]
...
process_piece_recursive(mesh, depth, timeout_at):
    ...
    return results
```
[Skiena Â§9.1 backtracking template; Skiena Â§9.4 look-ahead pruning;
Skiena Â§9.4 most-constrained-first; Bentley Ch.8 divide-and-conquer]

### Refinements from Distillations

- **Articulation vertex pre-check** (c-style-guide Â§7.8.1; Skiena Â§7.9.2):
  before running the expensive max-flow, check for articulation vertices
  with a linear-time DFS O(V+E). If one exists, cutting there is free â€” no
...
- **Oracle caching** (Skiena Â§9.3): if a sub-mesh was already tested by the
  ...
  levels Ã— 2 pieces Ã— 20 components, the calls add up.
...
- **Timeout discipline**: use `clock_gettime(CLOCK_MONOTONIC)` for the
  ...
  fires, return the mesh unchanged â€” partial results are worse than the
...
### Max-Flow Algorithm
- **CLRS Ch.26 Â§26.1â€“26.2**: flow network definitions, Ford-Fulkerson method,
  Edmonds-Karp algorithm (BFS augmenting paths), O(VEÂ²) bound (Theorem 26.8),
  max-flow min-cut theorem (Theorem 26.6). The full pseudocode in the
  distillation (clrs_rules, rule 28) maps directly to C.
- **CLRS Ch.26 Problem 26-1**: vertex splitting â€” the construction that
...
- **CLRS Ch.26.3 Theorem 26.10**: integrality theorem â€” since all capacities
  are integer, the max-flow is integer. Each augmenting path adds â‰¥ 1 unit.
  Number of BFS passes = flow value = min vertex cut size.
- **Skiena Ch.8.5 Â§8.5.2**: practical Edmonds-Karp implementation. The
...
- **Skiena Ch.15.4**: packed arrays for static graphs â€” exactly CSR.
  4Ã— speedup over LEDA's general graph type just from compactness.
- **c-style-guide Â§7.1**: CSR construction (two-pass: degree count â†’ prefix
  sum â†’ fill). Pre-allocate forward + reverse edges together.

### BFS
- **CLRS Ch.22.2**: BFS algorithm, O(V+E), shortest-path guarantee. Flat
  queue with head/tail indices. Eliminate the color array â€” `dist[v] == UINT32_MAX`
  means unvisited.
...
- **Skiena Ch.9.1 Â§9.1â€“9.3**: the five-callback backtracking template. Maps
  ...
  DFS recursion uses O(depth) space vs O(width) for BFS â€” critical since
  depth â‰¤ 10 but width could be 2^10 = 1024.
- **Skiena Ch.9.4**: look-ahead pruning (oracle check before recursing) +
  most-constrained-first (process larger piece first) together gave >1000Ã—
  speedup in the Sudoku experiment.
- **Skiena Ch.9.5**: "chessboard covering" war story â€” pruning at a higher
...
- **Skiena Ch.15.6**: KD-tree for nearest-neighbor queries. Build on boundary
  ...
  each query O(âˆšB). Or brute-force O(B) per query if B < 2000.
- **c-style-guide Â§6.4.1â€“6.4.6**: flat-array KD-tree, heap layout, compare
  squared distances, prune subtrees by bounding box.
- **Bentley Ch.9**: replace sqrt with squared-distance comparison.

### Performance and Cache
- **Abrash Ch.19 (c-style-guide Â§5.5.1â€“5.5.2)**: the BFS inner loop has
  unpredictable `visited[v]` branches. Consider branchless techniques or
  restructuring. Don't unroll loops â€” branch prediction handles the backwards
  branch perfectly.
- **Abrash Ch.15â€“17 (c-style-guide Â§5.4)**: working-set analysis. The flow
  network is ~85 MB for a 200K-vertex mesh â€” fits in Kaggle's L3 but is
  tight. If it spills to DRAM, BFS performance degrades catastrophically.
  Monitor cache miss rates with `perf stat`.
- **Bentley Ch.7 (c-style-guide Â§5.1)**: always do the arithmetic before
  writing the code. The feasibility estimates in Â§Performance Budget below
  are Bentley-style calculations.
- **Sanglard Ch.7 (c-style-guide Â§5.6.1)**: prefer a serial pipeline with
  parallelism inside each stage. Bridge cut is recursive â€” don't try to
  parallelize across recursion levels.

### Arena and Memory Management
- **Hanson Ch.6 (c-style-guide Â§1.1â€“1.2)**: arena allocation for all flow
...
### Error Handling
- **Hanson Ch.4 (c-style-guide Â§2.6)**: Exceptions (`TRY`/`EXCEPT`) for
  timeout and OOM. Checked runtime errors (`assert`) for contract violations.
  Never silently produce wrong output â€” assert and fail the volume.
...
**Step 2 budget**: 15â€“75s in Python. Target for C: **2â€“10s** per component
on Kaggle, depending on mesh size and recursion depth. For a volume with â‰¤ 5
multi-sheet components: **5â€“30s total.**
...
| k-ring BFS (SEED_RING=20) | O(ball) â‰ˆ 5K verts | ~0.05 ms |
| Build vertex-split CSR | O(N + E) â‰ˆ 400K ops | ~5 ms |
| Edmonds-Karp (100 BFS passes) | 100 Ã— O(V'+E') = 100 Ã— 2.6M | ~2.6 s |
| Path trace + augment (100Ã—) | 100 Ã— ~20 hops | ~0.2 ms |
| Min-cut extraction | O(V') = 200K | ~2 ms |
| is_bridge heuristic | O(1) | negligible |
| Build KD-tree (boundary, ~1K pts) | O(B log B) = 10K | ~0.1 ms |
| Nearest-boundary query (100K pts) | 100K Ã— O(âˆšB) â‰ˆ 100K Ã— 30 | ~3 ms |
...
â†’ ~5.2s per detection. This is the critical uncertainty.

### Recursive Depth Analysis

Typical case: 2â€“3 recursion levels per component (multi-sheet components
...
~N/2 verts, level 2 on ~N/4 verts. Total â‰ˆ 2N â€” approximately 2Ã— the cost
of a single detection.

Worst case: 10 recursion levels, each splitting roughly evenly. Total â‰ˆ 10Ã—
...
- **CPU speed**: 7950X single-core is ~2.6Ã— faster than Kaggle @ 2.2 GHz.
  A 2.6s detection on Kaggle â‰ˆ 1.0s on 7950X.
- **Cache**: 7950X L3 is 64 MB (shared) with 1 MB L2 per core. Kaggle's
  ...
  spill on Kaggle but fit in 7950X L3. This is a significant variable â€”
...
**Â§1.2 Arena Save/Restore** (c-style-guide): use `Arena_save` before
...
```
[Hanson Ch.6, Ch.16; Sanglard Ch.2; c-style-guide Â§1.2.2]

**Â§5.1 Feasibility Check**: done above. Single detection ~2.6s on Kaggle
...
[Bentley Ch.7; c-style-guide Â§7.4.2]

**Â§5.4 Working Set**: the flow network for N=100K is ~42 MB. On 7950X
...
[Abrash Ch.19; c-style-guide Â§5.4.1â€“5.4.7]

**Â§5.5 Branch Prediction**: the BFS inner loop's `if (residual > 0 && parent_edge[v] == -1)`
is an unpredictable branch on early iterations (roughly 50/50 whether a
neighbor is visited). On later iterations, most neighbors are visited â†’
the branch becomes predictable. Don't try to optimize this away â€” the
BFS is memory-bound, not branch-bound. Profile first.
[Abrash Ch.19; c-style-guide Â§5.5.2]

**Â§7.1 CSR Construction**: build the vertex-split flow graph in two passes.
...
[CLRS Ch.22.1; c-style-guide Â§7.1.3â€“7.1.5]

**Â§7.4.4 Reverse Edge Bug**: the #1 max-flow implementation bug. When
...
[CLRS Â§26.2; Skiena Â§8.5; c-style-guide Â§7.4.4]

**Â§9.3.2 Graceful Empty Cases**: handle every degenerate case:
- Zero-face mesh â†’ return unchanged.
- All vertices in one seed set (overlap removes everything) â†’ return NO BRIDGE.
- Flow = 0 (source disconnected from sink in residual graph) â†’ degenerate,
  return NO BRIDGE.
- Either sub-mesh has < 10 faces â†’ degenerate split, return original.
- Depth > 10 â†’ return unchanged.
[Elements Ch.4; c-style-guide Â§9.3.2]

**Â§9.4.4 Index Overflow**: the vertex-split graph has nodes indexed 0 to
...
[c-style-guide Â§9.4.4; Pitfalls â€” semantic]

**Â§2.6 Error Handling**: wrap the per-component bridge cut in a TRY-EXCEPT
...
[Hanson Ch.4; c-style-guide Â§2.6.1â€“2.6.6]
...
typedef struct FlowNet {
    ...
    int32_t *offset;           /* [n_nodes + 1] â€” CSR row offsets */
    int32_t *target;           /* [n_edges] â€” edge target node */
    int32_t *cap;              /* [n_edges] â€” edge capacity (1 or INF) */
    int32_t *flow;             /* [n_edges] â€” current flow (init 0) */
    int32_t *reverse;          /* [n_edges] â€” index of reverse edge */
...
```c
int32_t *parent_edge;   /* [n_nodes] â€” edge used to reach each node, -1 = unvisited */
int32_t *queue;         /* [n_nodes] â€” flat BFS queue */
```
`parent_edge` is `memset` to -1 at the start of each BFS. `parent_edge[source] = -2`
(visited sentinel, no parent). Reuse the same arrays for all 100 BFS passes â€”
no reallocation.

**Reachable array** (min-cut result):
```c
uint8_t *reachable;     /* [N] â€” per original vertex, 0 or 1 */
```
...
**Seed sets** (temporary):
```c
uint8_t *is_source_seed; /* [N] â€” bitmap */
uint8_t *is_sink_seed;   /* [N] â€” bitmap */
```
Populated by k-ring BFS, consumed during graph construction.

**Boundary vertex set** (for split_mesh):
```c
float   *boundary_pts;   /* [B * 3] â€” positions of boundary vertices */
int32_t *boundary_ids;   /* [B] â€” original vertex indices */
size_t   n_boundary;
```
...
with sheet_count â‰¥ 2 are routed to `BridgeCut_process`. Single-sheet
...
any sub-mesh still has sheet_count â‰¥ 2, it enters the raycast separator
...
1. **Flow value mismatch**: compare `total_flow` values. They must be
   identical â€” Edmonds-Karp is deterministic given the same graph.
...
- **Gray zone**: a mesh with flow_height_ratio between 0.36 and 1.3, with
  split_ratio â‰¥ 0.40 and height â‰¥ 20. Verify the gray-zone rule correctly
...
1. **Flow conservation**: for every node except S and T, verify
   Î£(flow on incoming edges) = Î£(flow on outgoing edges).
   [CLRS Â§26.1]
2. **Capacity constraint**: for all edges, verify `0 â‰¤ flow[e] â‰¤ cap[e]`.
   [CLRS Â§26.1]
3. **Max-flow = min-cut**: after extracting the reachable set, compute the
   ...
   [CLRS Â§26.2 Theorem 26.6]
4. **Reverse edge consistency**: for all edges e, verify
   `flow[e] + flow[reverse[e]] == 0`.

These checks are O(V+E) each â€” negligible compared to the BFS passes.
Leave them in until the flow implementation is proven correct on â‰¥ 20
test volumes.
...
### 1. The Reverse-Edge Bug (CLRS Â§26.2; Skiena Â§8.5; c-style-guide Â§7.4.4)
...
### 2. Flow Initialization (CLRS Â§26.2 rule 30)
...
[c-style-guide Â§3.3 â€” hide confusing expressions in named macros]

### 4. Seed Overlap (PIPELINE_REFERENCE Â§6 step 6)
...
â€” the flow is immediately INF (or MAX_FLOW_LIMIT), and every component
...
### 6. The â‰¤ vs < in is_bridge (Elements Ch.4 rule 25)

`flow_height_ratio â‰¤ 0.36` means BRIDGE. If you write `< 0.36`, exact
boundary cases (ratio exactly 0.36) are classified as NO BRIDGE. Write a
unit test for the boundary: flow=36, height=100 â†’ ratio=0.36 â†’ should be
BRIDGE. Same for `split_ratio < 0.20` (strict â€” exactly 0.20 IS a bridge).
The gray-zone rule uses `â‰¤ 1.3` AND `â‰¥ 0.40` â€” verify all four boundary
...
Use a large but not maximal value: `#define FLOW_INF (1 << 30)` (â‰ˆ 1 billion).
...
[Pitfalls â€” semantic; c-style-guide Â§9.5.1]
...
[c-style-guide Â§9.3.3 â€” asymmetric bounds]
...
```
[c-style-guide Â§2.6.2 â€” timeout exception]
...
are visited â€” which depends on CSR construction order. If the C CSR builds
...
This is not a bug â€” different min-cuts of the same value are all correct.
...
comparable.
[CLRS Â§26.2 â€” multiple optimal solutions]
