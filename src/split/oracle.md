# STAGE 1 â€” Oracle (Sheet Counter)
...
contains. This integer â€” the **sheet count** â€” is the single most important
...
with sheet count â‰¥ 2 enters the expensive topological repair path.

The Oracle is the pipeline's **high-level pruning gate**. Its cost is trivial
(1â€“2s in Python, sub-second in C) but its effect is enormous: it prevents
the pipeline from spending 15â€“75s on Edmonds-Karp max-flow or 30â€“110s on
per-vertex KD-tree queries for components that don't need it. Skiena Â§9.5's
chessboard covering war story is the exact analog â€” pruning at a higher
...
**Output**: integer sheet count (1, 2, 3, â€¦) for that component.
...
- A false positive (oracle says 2 sheets when there is 1) wastes time on
  bridge cut but is not catastrophic â€” bridge cut will find no bridge
...
1. **Reuse PCA normal** from Step 0's `ComponentMesh.pca_normal[3]`. Do NOT
   recompute â€” the normal is already computed and sign-corrected.
   [c-style-guide Â§6.6.3 â€” store intermediate forms for reuse;
   Abrash Ch.50 â€” avoid redundant recomputation]
...
3. **Shift vertices** so all coordinates are non-negative. Compute per-axis
   minimum, subtract from all vertices. This ensures voxel indices are â‰¥ 0
   when rounding. [PIPELINE_REFERENCE Â§5, step 2]
...
4. **Compute rasterization volume bounds**: from the shifted vertices, find
   ...
   `ceil(max_z)+1 Ã— ceil(max_y)+1 Ã— ceil(max_x)+1`. Allocate from arena
   as `uint8_t *rast_vol`, zero-initialized via `Arena_calloc`.
   [PIPELINE_REFERENCE Â§5, step 3]
...
5. **Rasterize each triangle** using barycentric sampling:
   ...
   ```
   This is a tight arithmetic loop â€” the Python version uses Numba JIT,
   the C version is a direct nested loop with no function calls in the
   inner body. [PIPELINE_REFERENCE Â§5, step 3; c-style-guide Â§6.5.1]
...
6. **Project all occupied voxels** to `(u, v, w)` coordinates:
   ...
   [PIPELINE_REFERENCE Â§5, step 4]

7. **Bin by (u, v)** on a `GRID_SIZE Ã— GRID_SIZE` grid (GRID_SIZE = 320):
...
8. **Per-cell gap counting**: for each non-empty cell, sort the w-values.
   ...
   [PIPELINE_REFERENCE Â§5, step 4; Bentley Ch.8 â€” scanning algorithm with
   running state]

### Phase 4: Aggregation
9. **Consensus voting**: build a histogram of per-cell sheet counts. Find the
   highest sheet count where â‰¥ `MIN_CELL_RATIO` (10%) of non-empty cells agree.
   Return that as the component's sheet count. If no count reaches 10%, return 1
   (single sheet). [PIPELINE_REFERENCE Â§5, step 5]

### Refinements from Distillations

- **Prefix-sum optimization** (Bentley Ch.2, c-style-guide Â§6.6.5): instead
  ...
  O(K log K) sort per cell. Profile both approaches â€” for the typical case
...
- **Resolution tuning** (sanglard_rules_part2 section N "Console Port Lessons", rule 41): the 320Ã—320 UV grid may be
  overkill. A 160Ã—160 grid has 4Ã— fewer cells and may detect the same
  multi-sheet components. Profile on training volumes with 160, 240, 320 grids
  and compare sheet-count accuracy. If 160 is as accurate, use it â€” it halves
...
### Core Algorithm
- **PIPELINE_REFERENCE Â§5**: the complete Oracle algorithm specification
...
- **Bentley Ch.8** (scanning algorithms): the per-cell gap counting is a
  classic scanning algorithm â€” sort once, scan linearly maintaining a running
...
- **Bentley Ch.2** (prefix sums): the cumulative-histogram optimization for
  gap counting is a direct application of Bentley's prefix-sum pattern â€”
...
- **Skiena Ch.9.4** (Sudoku): look-ahead pruning + most-constrained-first
  together gave >1000Ã— speedup. The Oracle is the look-ahead: check the
...
- **Abrash Ch.51** (backface removal): "instant rejection of half the work
  ...
  sheet components from the expensive path â€” potentially 50%+ of components.
...
### Rasterization Loop
- **Abrash Ch.38â€“39** (polygon fill): the rasterization phase is a direct
  analog of Abrash's scan-conversion â€” iterate over barycentric coordinates,
...
- **c-style-guide Â§6.5.1**: barycentric subdivision pattern for mesh
...
- **Abrash Ch.19** (branch prediction): the rasterization inner loop has a
  predictable branch (bounds check â€” almost always true). Don't unroll. The
...
- **Abrash Ch.61** (frame of reference): the UV basis construction is a
  coordinate frame change. The dot product is the universal workhorse â€” three
...
- **Abrash Ch.74**: "the dot product is the universal workhorse of 3D
  geometry." Every operation in the Oracle â€” basis construction, projection,
  gap measurement â€” reduces to dot products.
...
- **Bentley Ch.8** (Column 8): the per-cell sort of w-values is on small arrays
  (typically 1â€“50 elements). Use insertion sort for N â‰¤ ~20, `qsort` for
  larger. Or since the per-cell count is almost always small, just use
  insertion sort unconditionally â€” it's optimal for small N and has no
  function-call overhead.
- **Skiena Ch.3.6** (bounded-height PQ): since sheet counts are small integers
  (typically 1â€“5), the aggregation histogram is a tiny array indexed by sheet
  count. No need for any sorting structure â€” direct array indexing.
...
**Step 1 budget**: ~2s (from the Python profile: 1â€“2s, bottleneck is
Numba-JIT rasterization). In C, expect significant speedup over the JIT â€”
...
|---|---|---|
| UV basis construction | O(1) â€” 2 cross products | negligible |
| Vertex shift | N vertices Ã— 3 subtracts | ~0.001s |
| Rasterize (per component) | M faces Ã— subdivÂ² samples Ã— 6 ops | see below |
| Voxel projection | V_occupied Ã— 3 dot products | see below |
| UV binning | V_occupied Ã— 2 divides + array write | see below |
| Per-cell sort | Î£(K_cell Ã— log K_cell) over all cells | see below |
| Aggregation | O(n_cells) scan | negligible |

**Rasterization detail**: a typical component has M â‰ˆ 100K faces (after
backface cull). Average `subdiv` â‰ˆ 8 (for faces with max edge length ~8
voxels). Barycentric samples per face â‰ˆ subdivÂ²/2 â‰ˆ 32. Total samples:
100K Ã— 32 = 3.2M. Per sample: 3 multiplies + 3 adds (barycentric interp)
+ 3 rounds + 1 bounds check + 1 array write â‰ˆ 15 ops. Total: 3.2M Ã— 15
= 48M ops Ã— ~0.5 ns/op = ~24 ms. **Rasterization is cheap.**

**Projection detail**: occupied voxels V_occ â‰ˆ 100Kâ€“500K (depending on
component size and surface density). Per voxel: 3 dot products (9 muls +
6 adds) + 2 divides (for UV binning) â‰ˆ 20 ops. Total: 500K Ã— 20 = 10M ops
Ã— ~0.5 ns = ~5 ms. **Projection is cheap.**

**Sorting detail**: 320Â² = 102,400 cells. Average occupancy â‰ˆ 5 voxels/cell
(500K voxels / 100K non-empty cells). Insertion sort on 5 elements: ~10
comparisons. Total: 100K cells Ã— 10 = 1M ops Ã— ~1 ns = ~1 ms. Even with
`qsort` overhead, this is negligible.

**Total per component**: ~30â€“50 ms. **For 20 components: ~0.6â€“1.0s.**

**Memory**: the temporary rasterization volume for a 320Â³ bounding box is
32.8 MB (uint8). The (u,v,w) projection array is up to 500K Ã— 12 bytes =
...
- Single-core 7950X is ~2.6Ã— faster than Kaggle.
- Expect ~0.2â€“0.4s on 7950X, ~0.6â€“1.0s on Kaggle.
- No parallelism needed within the Oracle â€” it's already fast enough.
...
**Â§1.2 Arena Lifetime** (c-style-guide): the rasterization volume, projection
...
**Â§1.6 Stack Hazards**: the rasterization volume for a 320Â³ component could
be ~35 MB. Never allocate on the stack. Always arena.

**Â§4.6.1 Dense Arrays**: vertices as `float verts[N*3]`, the rasterization
volume as `uint8_t rast_vol[D*H*W]`, the UV bins as flat arrays. No linked
structures, no per-cell malloc.

**Â§5.1 Feasibility Check**: done above. Step 1 fits with >100Ã— headroom.
No optimization needed. Write the simplest correct code.

**Â§5.2.1 Correctness First**: "The first principle of optimization is don't."
...
Step 2â€“3 is harmless (wasted time). A wrong sheet count that prevents a
multi-sheet bridge from entering Step 2 is catastrophic (wrong topology).

**Â§5.4.6 Reducing Space**: use `uint8_t` for the rasterization volume (binary:
0 or 1). Not `int32_t`. The 32.8 MB volume fits in L3 at uint8; it would be
131 MB at int32 and spill to DRAM.

**Â§5.5.1 No Loop Unrolling**: the rasterization inner loop is already
branch-predicted perfectly. Don't unroll. Don't add SIMD. Write clean scalar C.

**Â§6.1.1 Volume Layout**: `rast_vol[z*H*W + y*W + x]`. Same convention as
everywhere else. Document it.

**Â§6.6.3 Store Intermediate Forms**: the PCA normal from Step 0 is reused.
...
**Â§9.3.2 Graceful Empty Cases**: zero vertices â†’ return sheet_count=1. Zero
occupied voxels after rasterization â†’ return 1. Zero non-empty UV cells â†’
return 1. Never crash on degenerate input.

**Â§9.4.3 Integer Arithmetic for Counting**: sheet counts, cell counts, gap
counts â€” all integers. The `w` values and UV coordinates are float. The gap
comparison `w[i+1] - w[i] > MIN_GAP` uses float comparison with integer
threshold (10.0). No exact-equality comparison needed.

**Â§9.5 UB Avoidance**: the rasterization volume index
...
*Approach B* â€” bin directly during projection (avoids the intermediate array):
...
```
Build in two passes: pass 1 counts per-cell occupancy â†’ prefix sum for offsets.
...
[c-style-guide Â§7.1.3; Sanglard Ch.5 Â§6.4.8]

**Approach B is preferred** â€” it avoids the intermediate UVW array and produces
...
```c
int32_t sheet_hist[MAX_SHEET_COUNT + 1];  /* on the stack â€” MAX_SHEET_COUNT â‰ˆ 10 */
```
This is tiny (â‰¤ 44 bytes). Stack allocation is fine.
...
```c
/*
 * Oracle_count_sheets â€” Step 1 entry point.
 *
 ...
 *
 * Returns the sheet count (â‰¥ 1). Returns 1 for degenerate inputs.
 *
...
A single `int` â€” the sheet count. No heap allocation survives this call.

### Connection to Step 0 (upstream)

The Oracle receives `ComponentMesh` structs from Step 0. It reads:
- `mesh->verts`, `mesh->n_verts` â€” vertex positions
- `mesh->faces`, `mesh->n_faces` â€” triangle indices
- `mesh->pca_normal` â€” the PCA sheet normal (precomputed, sign-corrected)
...
for (size_t i = 0; i < n_meshes; i++) {
    ...
    /* else: single sheet, skip to Step 4 (rebuild) â†’ Step 5 (Poisson) */
}
```
...
recursing. [Skiena Ch.9.4 â€” look-ahead pruning]
...
This is by design â€” the Oracle must be fast, simple, and self-contained.
...
Sheet count is an integer â€” there is no tolerance. If they disagree, one of
them is wrong.
...
- **Borderline gap**: two planes separated by exactly 10 voxels.
  ...
  NOT a gap (> 10, not â‰¥ 10). Verify this boundary. A gap of 11 IS a gap.
  [Elements Ch.4 â€” branch the right way on equality]
...
Run the full pipeline on 5â€“10 representative training volumes. For each
...
gap = 10.01 (yes).
[Elements Ch.4 â€” "take care to branch the right way on equality";
c-style-guide Â§9.3.4]
...
produce negative indices â€” array underflow, silent memory corruption.
[c-style-guide Â§9.3.3 â€” off-by-one; Pitfalls â€” semantic]
...
too high, you waste cycles. The Python code uses `ceil(max_edge)` â€” match
...
[c-style-guide Â§9.3.3; Pitfalls â€” semantic]
...
in one UV direction).
[c-style-guide Â§9.4.2 â€” never compare float for exact equality;
guard divisions against zero]

### Rasterization Volume Memory for Large Components
A component whose bounding box spans the full 320Â³ volume requires a 32.8 MB
temporary volume. This is fine. But if a volume is 512Â³ and a component spans
...
and doubling MIN_GAP to 20 â€” same detection quality, 8Ã— less memory.
[c-style-guide Â§1.1.5 â€” arena sizing; sanglard_rules_part2 section N "Console Port Lessons", rule 41 â€” reduce resolution
...
can overflow `int32_t` for large volumes. For a 400Â³ volume:
400 Ã— 400 Ã— 400 = 64M â€” fits int32 (max 2.1B) but compute the intermediate
product `iz * H_rast * W_rast` in `size_t` to be safe. Use `size_t` for all
index arithmetic as mandated by c-style-guide Â§9.4.4.
...
elongated ones.
[PIPELINE_REFERENCE Â§16 â€” coordinate convention]
